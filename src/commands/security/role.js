...
module.exports = {
    ...
    async execute(message, client, args) {
        ...
        if (!hierarchy.ok) {
            return message.reply(`${ERROR_EMOJI} **ɪ ᴄᴀɴ'ᴛ ɢɪᴠᴇ ʀᴏʟᴇꜱ ᴛᴏ ᴛʜɪꜱ ᴜꜱᴇʀ.**`);
        }
        ...
        if (role.managed || role.name === '@everyone') {
            return message.reply(`${ERROR_EMOJI} **ɪ ᴄᴀɴ'ᴛ ɢɪᴠᴇ ᴛʜɪꜱ ʀᴏʟᴇ.**`);
        }
        ...
        if (!role.editable || (botMember.roles.highest?.position ?? 0) <= role.position) {
            return message.reply(`${ERROR_EMOJI} **ɪ ᴄᴀɴ'ᴛ ɢɪᴠᴇ ᴛʜɪꜱ ʀᴏʟᴇ (ʜɪᴇʀᴀʀᴄʜʏ).**`);
        }

        if (targetMember.roles.cache.has(role.id)) {
            return message.reply(`${ERROR_EMOJI} **ᴛʜɪꜱ ᴜꜱᴇʀ ᴀʟʀᴇᴀᴅʏ ʜᴀꜱ ᴛʜᴀᴛ ʀᴏʟᴇ.**`);
        }

        try {
            await targetMember.roles.add(role, `Prefix role assignment by ${message.author.tag}`);

            return message.reply(`${DONE_EMOJI} **ᴅᴏɴᴇ, ${targetMember} ɢᴏᴛ ᴛʜᴇ ʀᴏʟᴇ (${String(role?.name || 'role').toUpperCase()}).**`);
        } catch (e) {
