...
module.exports = {
    ...
    async execute(message) {
        ...
        if (!mutedRole) {
            return message.reply(`${ERROR_EMOJI} **ɴᴏ ᴍᴜᴛᴇ ʀᴏʟᴇ ꜰᴏᴜɴᴅ.**`);
        }

        if (!target.roles.cache.has(mutedRole.id)) {
            return message.reply(`${ERROR_EMOJI} **ᴛʜɪꜱ ᴜꜱᴇʀ ɪꜱ ɴᴏᴛ ᴍᴜᴛᴇᴅ.**`);
        }

        try {
            await target.roles.remove(mutedRole, `Unmuted by ${message.author.tag}`);
            return message.reply(`${DONE_EMOJI} **ᴅᴏɴᴇ, ${target} ʜᴀꜱ ʙᴇᴇɴ ᴜɴᴍᴜᴛᴇᴅ.**`);
        } catch (e) {
